...
Public Class FrmDrawnNameMstr
        ...
        If BtnSave.Text = "&Save" Then
            ...
            If SveCnfrm = vbYes Then
                ...
                If addFlag = True Then
                    CatCmd = New SqlCommand("Finact_DrawnNameMstr_Insert", FinActConn)
                ...
                ElseIf EdtFlag = True Then
                    CatCmd = New SqlCommand("Finact_DrawnNameMstr_Update", FinActConn)
                    CatCmd.CommandType = CommandType.StoredProcedure
                    CatCmd.Parameters.AddWithValue("@Nrr", Trim(TxtDept.Text))
    ...
    Private Sub BtnFind_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnFind.Click
        If BtnFind.Text = "&Find" Then
            ...
            CatCmd = New SqlCommand("Select * from Finact_DrawnNameMstr where Drwndelstatus=1 order by DrwnName ", FinActConn)
            Try
                DtaRdr = CatCmd.ExecuteReader
                Dim x As Integer
                x = 0
                While DtaRdr.Read()
                    LstItem = LstVewFnd.Items.Add(DtaRdr("DrwnName"), 1)
                    LstVewFnd.Items(x).SubItems.Add(DtaRdr("Drwnid"))
                    x += 1
                End While
            Catch ex As Exception
                MsgBox(ex.Message)
            ...
            If TxtDept.Text <> "" And EdtRecrdNo > 0 Then
                Select Case MsgBox("Are you sure to delete this record?", MsgBoxStyle.Question + MsgBoxStyle.YesNo)
                    Case MsgBoxResult.Yes
                        CatCmd = New SqlCommand("Delete from Finact_DrawnNameMstr where Drwnid=@deptid", FinActConn)
