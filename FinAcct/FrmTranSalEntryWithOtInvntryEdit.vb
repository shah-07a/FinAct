...
Imports System.IO
Public Class FrmTranSalEntryWithOtInvntryEdit
    ...
    Private Sub FrmTranSaleOrder_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If xWrkContrctSale_Flag = True Then
                ...
                Me.Text = "Sale Billing (Job/Work-Contract) Edit Mode" & "  (CURRENT USER NAME:- " & Trim(Current_UserName) & ",DESIGNATION:- " & Trim(Current_UserDesi)
            ...
            Else
                ...
                Me.Text = "Sale Billing (Without Inventory) Edit Mode" & "  (CURRENT USER NAME:- " & Trim(Current_UserName) & ",DESIGNATION:- " & Trim(Current_UserDesi)
                Me.Label42txabl.Text = " = Taxable Amt."
            End If
            If xWrkContrctSale_Flag = True Then
                Fill_Combobox_where_cond("Saleentid", "Saleentvno", "SaleentBILLtype", "FinactSaleentry", "SALEENTDELSTATUS", CInt(1), "JBWRK", Me.CmbxBillno)
            Else
                Fill_Combobox_where_cond("Saleentid", "Saleentvno", "SaleentBILLtype", "FinactSaleentry", "SALEENTDELSTATUS", CInt(1), "NOINV", Me.CmbxBillno)
            End If
            CreateGridColumns()
    ...
    , Txtgrno.KeyDown, TxtinsCo.KeyDown, TxtinsCo.KeyDown, TxtPlcyno.KeyDown, TxtPvtMrk.KeyDown, TxtGrsWt.KeyDown _
    ...
    Private Sub BtnPe_Save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSe_Save.Click
        Try
            ...
            If MessageBox.Show("Are you sure to update this record", "Sale Entry updating......", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.No Then
                BtnSe_Save.Focus()
                Return
            Else
                SaleEntryupdate()
    ...
    Private Sub SaleEntryupdate()
        If Me.DgSaleDirect.Rows.Count > 0 Then
            Try
                ...
                TpoCmd.Parameters.AddWithValue("@Saleentid", CInt(Me.CmbxBillno.SelectedValue))
                ...
                If xWrkContrctSale_Flag = True Then
                    ...
                    TpoCmd.Parameters.AddWithValue("@SaleentadnldisRate", CDbl(0))
                    TpoCmd.Parameters.AddWithValue("@SaleentAdnlDis", CDbl(0))
                Else
                    ...
                    TpoCmd.Parameters.AddWithValue("@saleENTJOBRATE", CDbl(0))
                    TpoCmd.Parameters.AddWithValue("@saleENTJOBAMT", CDbl(0))
                End If
            ...
            Try
                ...
                For DgCntr = 0 To Me.DgSaleDirect.Rows.Count - 1
                    If Me.DgSaleDirect.Rows(DgCntr).Cells(4).Value = 1 Then
                        TpoCmd = New SqlCommand("Finact_Saleentry_Details_update", FinActConn)
                        TpoCmd.CommandType = CommandType.StoredProcedure
                        TpoCmd.Parameters.AddWithValue("@dSaleentconcrnid", CInt(Me.CmbxBillno.SelectedValue))
                        ' TpoCmd.Parameters.AddWithValue("@dSaleent_con_dSaleid", 0)
                        TpoCmd.Parameters.AddWithValue("@dSaleentid", (Me.DgSaleDirect.Rows(DgCntr).Cells(5).Value))
                        ...
                        TpoCmd.Parameters.AddWithValue("@dSaleentlocid", CInt(Me.CmbxWareh.SelectedValue))
                        TpoCmd.Parameters.AddWithValue("@SaleEntCartNo", (Me.DgSaleDirect.Rows(DgCntr).Cells(6).Value))
                        TpoCmd.ExecuteNonQuery()
                    ElseIf Me.DgSaleDirect.Rows(DgCntr).Cells(4).Value = 2 Then
    ...
    Private Sub CreateGridColumns()
        ...
        '5
        Me.DgSaleDirect.Columns.Add("Rec_Id", "Recid")
        Me.DgSaleDirect.Columns("Rec_Id").Width = 0
        Me.DgSaleDirect.Columns("Rec_Id").Visible = False
        '6
        ...
        Try
            If e.KeyCode = Keys.Enter Or e.KeyCode = Keys.Tab Then
                If Me.DgSaleDirect.CurrentCell.ColumnIndex = 2 Then
                    Me.DgSaleDirect.CurrentCell = Me.DgSaleDirect.Item(6, Me.DgSaleDirect.CurrentCell.RowIndex)
                    Exit Sub
                End If

                If Me.DgSaleDirect.CurrentCell.ColumnIndex = 6 Then ' Me.DgSaleDirect.ColumnCount = 3 Then
                    If IsDBNull(Me.DgSaleDirect.CurrentRow.Cells(0).Value) = False And IsDBNull(Me.DgSaleDirect.CurrentRow.Cells(1).Value) = False And IsDBNull(Me.DgSaleDirect.CurrentRow.Cells(3).Value) = False Then
                        Dim Val1 As Double
                        Dim ic As String
                        ...
                        Else
                            ...
                            Else
                                ...
                                Me.SaveFlag = True
                                If Me.DgSaleDirect.CurrentRow.Index = Me.DgSaleDirect.RowCount - 1 Then
                                    If MessageBox.Show("Do you want to add more items?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                                        If Me.DgSaleDirect.CurrentRow.Index = Me.DgSaleDirect.Rows.Count - 1 Then
                                            ...
                                            Me.DgSaleDirect.Rows(xsno - 1).Cells(4).Value = 2
                                        End If
                                    Else
                    ...
                    If Me.DgSaleDirect.CurrentCell.RowIndex < Me.DgSaleDirect.RowCount - 1 Then
                        ...
                        If Me.DgSaleDirect.CurrentRow.Cells(4).Value = 2 Then
    ...
    Private Sub BtnAll_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSe_Save.GotFocus, BtnSe_exit.GotFocus, BtnNew.GotFocus
        Try
    ...
    Private Sub BtnAll_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSe_exit.Leave, BtnSe_Save.Leave, BtnNew.Leave
        Try
            Dim xBtn As Button = CType(sender, Button)
        ...
        Try
            If CheckBlank_Cmbx(Me.CmbxBillno) = True Then
                If Me.CmbxBillno.SelectedIndex = 0 Then
                    xFin_n_Fill_Saleentry_Edit(Me.CmbxBillno.SelectedValue)
                End If
                x_SBillNo_x = Me.CmbxBillno.Text.Trim
    ...
    Private Sub xFin_n_Fill_Saleentry_Edit(ByVal xBillid As Integer)
        Dim CurrCulture As System.Globalization.CultureInfo = System.Globalization.CultureInfo.CurrentCulture
        Dim CurrDate As Date
        Try
            TpoCmd1 = New SqlCommand("Finact_SaleEntry_Select_Where_id", FinActConn1)
            ...
            If TpoRdr1.IsDBNull(0) = False Then
                ...
                Me.CmbxCust.SelectedValue = CInt(TpoRdr1("Saleentsplrid"))
                ...
                Me.lblgross.Text = FormatNumber(TpoRdr1("Saleenttotlamt") - TpoRdr1("SaleEntRondOff"), 2)
                ...
                If xWrkContrctSale_Flag = True Then
                    Me.MskAdlDis.Text = FormatNumber(TpoRdr1("saleENTJOBRATE"), 2)
                    Me.LblAdlDis.Text = FormatNumber(TpoRdr1("saleENTJOBAMT"), 2)
                Else
                    If TpoRdr1("SaleentAdnlDisType") = True Then
                ...
                End If
                Me.CmbxStokGrup.SelectedValue = CInt(TpoRdr1("SaleEnt_UGRUP"))

                Me.CmbxAgent.SelectedValue = TpoRdr1("SaleentAgntId")

                Dim xFrgt As Integer = 0
                If xFrgt = 0 Then
        ...
        End Try
        Try
            TpoCmd1 = New SqlCommand("Finact_SaleEntry_Details_Where_SaleEntid", FinActConn1)
            TpoCmd1.CommandType = CommandType.StoredProcedure
            TpoCmd1.Parameters.AddWithValue("@xSaleEntid", CInt(Me.CmbxBillno.SelectedValue))
            TpoRdr1 = TpoCmd1.ExecuteReader
            Me.DgSaleDirect.Rows.Clear()
            ...
            While TpoRdr1.Read
                If TpoRdr1.IsDBNull(0) = False Then
                    Me.DgSaleDirect.Rows.Add()
                    Me.DgSaleDirect.Rows(Dinx).Cells(0).Value = FormatNumber(TpoRdr1("dSaleentqntyissue"), 3)
                    Me.DgSaleDirect.Rows(Dinx).Cells(1).Value = Trim(TpoRdr1("dSaleentDescrpt"))
                    Me.DgSaleDirect.Rows(Dinx).Cells(2).Value = FormatNumber(TpoRdr1("dSaleentitmrate"), 2)
                    Me.DgSaleDirect.Rows(Dinx).Cells(3).Value = FormatNumber(CDbl(TpoRdr1("dSaleentqntyissue") * TpoRdr1("dSaleentitmrate")), 2)
                    Me.DgSaleDirect.Rows(Dinx).Cells(4).Value = 1
                    Me.DgSaleDirect.Rows(Dinx).Cells(5).Value = CInt(TpoRdr1("DsaleEntid"))
                    Me.DgSaleDirect.Rows(Dinx).Cells(6).Value = CInt(TpoRdr1("SaleEntCartNo"))
                    Dinx += 1
                End If
            End While
            Me.DgSaleDirect.MultiSelect = False
            Me.DgSaleDirect.AllowUserToDeleteRows = False
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
            TpoCmd1.Dispose()
    ...
    Private Sub BtnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNew.Click
        Try
            Dim xS As New FrmTranSalEntryWithOtInvntry
  ...
  , Mskothrchrg.GotFocus, MskPostcharg.GotFocus, MtxtfrtChargs.GotFocus, mtxtulcharg.GotFocus
   ...
   , Cmbxstatus.Leave, MTxtTotlamt.Leave
    ...
    Private Sub TxtinsCo_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtinsCo.TextChanged
