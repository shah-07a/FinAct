...
Public Class FrmCrRptPackNote
    ...
    Private Sub frmcrrptsaleorder_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        cmd = New SqlCommand("Delete from FINACT_TEMP_PurOrder", FinActConn)
        Try
            cmd.ExecuteNonQuery()
    ...
    Private Sub frmcrrptsaleorder_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Fill_Combobox("saleid", "saleordno", "finactsaleorder", "SaleDelStatus", CInt(1), Me.CmbxLdgr)
            FillTempTable_PurOrder()
            If xPackNoteFlag = True Then
                xPackNoteFlag = False
                Me.DtpLdgr1.Value = xPackNoteDt
                Me.DtpLdgr2.Value = xPackNoteDt
                Me.CmbxLdgr.SelectedValue = xPackNoteNO
    ...
    End Sub
    Private Sub FillTempTable_PurOrder()
        Try
            If FinActConn.State Then FinActConn.Close()
            FinActConn.Open()
            If xPackNoteFlag = True Then
                cmd = New SqlCommand("Finact_Rpt_SaleOrder_for_PackList_KBLtype_FillTalbe", FinActConn)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.AddWithValue("@Soordid", CInt(xPackNoteNO))
            Else
                cmd = New SqlCommand("Finact_Rpt_SaleOrder_for_PackList_FillTalbe", FinActConn)
                cmd.CommandType = CommandType.StoredProcedure
            End If
    ...
    Private Sub BtnRptVewLdgr_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnRptVewLdgr.Click
        Try
            ...
            SelRecd_Fromtable_PurOrderSelect()
            'FrmMainMdi.MenuItem261.Enabled = False
            Handle_P_Bar(Me)
            Me.Size = New Point(890, Screen.PrimaryScreen.WorkingArea.Height - 50)
            Me.Top = 0
            Me.Left = 0
        Catch ex As Exception
        ...
        End Try
    End Sub
    Private Sub SelRecd_Fromtable_PurOrderSelect()
        Try
            Dim CrRptSaleOrd As Object
            If Cl_Kbl = True Then
                CrRptSaleOrd = New CrRpt_KBL_SaleOrder
            ElseIf Cl_TimeIndia = True Or Cl_TimeRubber = True Then
                CrRptSaleOrd = New CrRptPackNote
            End If
            ...
            Fetch_Stock_Record_SelectedDateRange_withSelITem_and_BalType("Finact_Rpt_TEMP_PurOrder_Select", CrRptSaleOrd, Me.CrRptVewSaleOrdr, Me.DtpLdgr1.Value.Date, Me.DtpLdgr2.Value.Date, Selact, Stype, Sbtype)

            If Me.ChkRptLdgr.Checked = True Then
                CoprofileParamsRest_Adrs(CrRptSaleOrd, Me.CrRptVewSaleOrdr, True, True, Me.DtpLdgr1, Me.DtpLdgr2)
            Else
                CoprofileParamsRest_Adrs(CrRptSaleOrd, Me.CrRptVewSaleOrdr, True, False, Me.DtpLdgr1, Me.DtpLdgr2)
            End If
