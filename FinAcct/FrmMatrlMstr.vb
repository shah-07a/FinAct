...
Public Class FrmMatrlMstr
    ...
    Private Sub Save_Recd()
        Try
            If Trim(BtnSave.Text) = "&Save" Then
                Itmconfig_cmd = New SqlCommand("finact_itmmatrlmstr_insert", FinActConn)
                Itmconfig_cmd.CommandType = CommandType.StoredProcedure
            End If
            Itmconfig_cmd.Parameters.AddWithValue("@mname", Txthtin.Text)
            ...
            Try
                If dg.SelectedRows.Count > 0 Then
                    ...
                    For i = 0 To dg.SelectedRows.Count - 1
                        Itmconfig_cmd = New SqlCommand("Delete from finact_itmmatrlmstr where itmgradid=@itmid", FinActConn)
    ...
    Private Sub Create_fill_datagrid()
        Try
            ...
            dg.Columns.Add("in", "Name Of Material")
    ...
    Private Function validate_input() As Boolean
        ...
        For i = 0 To Me.dg.Rows.Count - 1
            For j = i + 1 To Me.dg.Rows.Count - 1
                If Me.dg.Rows(i).Cells(1).Value = Me.dg.Rows(j).Cells(1).Value Then ' And Me.dg.Rows(i).Cells(2).Value = Me.dg.Rows(j).Cells(2).Value Then
                    Return True
                End If
            Next
        Next
            ...
            If b > 0 Then
                For i = 0 To dg.SelectedRows.Count - 1
                    ...
                    If FlagEditAllow = True Then
                        ...
                        End If
                    End If
                    Itmconfig_cmd = New SqlCommand("Finact_itmmatrlmstr_update", FinActConn)
                    Itmconfig_cmd.CommandType = CommandType.StoredProcedure
                    Itmconfig_cmd.Parameters.AddWithValue("@itmId", Me.dg.SelectedRows(i).Cells(0).Value)
                    Itmconfig_cmd.Parameters.AddWithValue("@mname", Me.dg.SelectedRows(i).Cells(1).Value)
    ...
    Private Function ChkExisiting_Rec() As Boolean
        Try
            Itmconfig_cmd = New SqlCommand("select itmmatrlname from finact_itmmatrlmstr where itmmatrlname=@in", FinActConn)
