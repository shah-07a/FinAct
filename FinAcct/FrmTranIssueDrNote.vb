...
Public Class FrmTranIssueDrNote
    ...
    Private Sub FrmTranSaleOrder_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Me.Text = "Item(s) Rejected/Received Back " & " Current User :- " & Trim(Current_UserName) & " - " & Trim(Current_UserDesi)
            ...
            Me.TxtSaleBillNo.Text = CInt(xFetchVno_dynamic("FinactsaleItemBackRecdEntry", "Sbr_entvno"))
            ...
            If Me.tplcustlist.Visible = True Then Me.tplcustlist.Visible = False
            Me.dtpordrdt.Focus()
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub find_Customer_list(ByVal CurString As String)
        ...
        Try
            TpoCmd = New SqlCommand("FinAct_SplrMstr_SelectRecHavingSaleBills_Like", FinActConn)
    ...
    Private Sub AllControls_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Chkprnt.KeyDown _
      ...
      , RbxiName.KeyDown, RbxName.KeyDown, Txtcoment.KeyDown, TxtSaleBillNo.KeyDown, TxtVatinvno.KeyDown, txtvCode.KeyDown, TxtVname.KeyDown
        Try
    ...
    Private Sub SaleEntrySave()
        If Me.DgSaleDirect.Rows.Count >= 1 Then
            Try
                Dim ChkQlty As String = ""
                If Me.RbTrt1.Checked = True Then
                    ChkQlty = "1stQlty"
                ElseIf Me.RbTrt2nd.Checked = True Then
                    ChkQlty = "2ndQlty"
                Else
                    ChkQlty = "MixQlty"
                End If
                TpoCmd = New SqlCommand("finact_SaleItemBackRecdEntry_insert", FinActConn)
                TpoCmd.CommandType = CommandType.StoredProcedure
                ...
                TpoCmd.Parameters.AddWithValue("@SBR_plistid", Me.CmbxPlist.SelectedValue)
                ...
                TpoCmd.Parameters.AddWithValue("@SBREnt_GrupId", CInt(Me.CmbxLdgrHead.SelectedValue))
            ...
            Try
                ...
                For DgCntr = 0 To Me.DgSaleDirect.Rows.Count - 1
                    If Me.DgSaleDirect.Rows(DgCntr).Cells(6).Value = 1 Then
                        ...
                        If Me.RbTrt2nd.Checked = True Then
                            TpoCmd = New SqlCommand("Finact_SaleItemBackRecdEntry_Details_Insert", FinActConn)
                            TpoCmd.CommandType = CommandType.StoredProcedure
                            TpoCmd.Parameters.AddWithValue("@d_SBR_entconcrnid", (CurrPbillId))
                            TpoCmd.Parameters.AddWithValue("@d_SBR_ent_con_d_SBR_id", 0)
                            TpoCmd.Parameters.AddWithValue("@d_SBR_entitmid", xxItemId)
                            TpoCmd.Parameters.AddWithValue("@d_SBR_entqnty", (Me.DgSaleDirect.Rows(DgCntr).Cells(0).Value))
                            TpoCmd.Parameters.AddWithValue("@d_SBR_entitmrate", (Me.DgSaleDirect.Rows(DgCntr).Cells(3).Value))
                            TpoCmd.Parameters.AddWithValue("@d_SBR_entlocid", Me.CmbxWareh.SelectedValue)
                            TpoCmd.ExecuteNonQuery()
                            TpoCmd.Dispose()
                        ElseIf Me.RbTrt1.Checked = True Then
                            If ChkExistingInBom(xxItemId) = False Then
                                TpoCmd = New SqlCommand("Finact_SBR_FirstQltyOtherThanBom_Details_Insert", FinActConn)
                                TpoCmd.CommandType = CommandType.StoredProcedure
                                ...
                                TpoCmd.Parameters.AddWithValue("@dPurentqnty", (Me.DgSaleDirect.Rows(DgCntr).Cells(0).Value))
                                ...
                                TpoCmd.Dispose()
                            Else
                                TpoCmd = New SqlCommand("Finact_SBR_FirstQlty_Details_Insert", FinActConn)
                                TpoCmd.CommandType = CommandType.StoredProcedure
                                TpoCmd.Parameters.AddWithValue("@dSaleentconcrnid", (CurrPbillId))
                                TpoCmd.Parameters.AddWithValue("@dSaleent_con_dSaleid", 0)
                                TpoCmd.Parameters.AddWithValue("@dSaleentitmid", xxItemId)
                                TpoCmd.Parameters.AddWithValue("@dSaleentqnty", (Me.DgSaleDirect.Rows(DgCntr).Cells(0).Value))
                                TpoCmd.Parameters.AddWithValue("@dSaleentitmrate", (Me.DgSaleDirect.Rows(DgCntr).Cells(3).Value))
                                TpoCmd.Parameters.AddWithValue("@dSaleentlocid", Me.CmbxWareh.SelectedValue)
                                TpoCmd.Parameters.AddWithValue("@dSaleent_Isprod", 2)
                                TpoCmd.ExecuteNonQuery()
                                TpoCmd.Dispose()
                            End If
            ...
            Catch ex As Exception
                ...
                Try
                    TpoCmd1 = New SqlCommand("Finact_SBR_Del_Ifcommandisunsuccess_Delete", FinActConn1)
                    TpoCmd1.CommandType = CommandType.StoredProcedure
    ...
    Private Function FindlastId(ByVal P_billno As String, ByVal Splr_id As Integer) As Integer
        Try
            ...
            TpoCmd1 = New SqlCommand("select SBR_entid from FinactSaleItemBackRecdEntry where SBR_entvno=@Ono and SBR_entsplrid=@sid", FinActConn1)
    ...
    Private Function Curr_MaxId() As Integer
        Try
            ...
            TpoCmd1 = New SqlCommand("select max(SBR_entid) from FinactSaleItemBackRecdEntry ", FinActConn1)
            TpoRdr1 = TpoCmd1.ExecuteReader
            TpoRdr1.Read()
            If TpoRdr1.IsDBNull(0) = False Then
    ...
    Private Function Curr_Maxdate() As Date
        Try
            ...
            FinActConn1.Open()
            TpoCmd1 = New SqlCommand("select max(SBR_entdt) from FinactSaleItemBackRecdEntry where  SBR_entdelstatus=1", FinActConn1)
    ...
    Private Sub Cmbxspcatid_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Cmbxspcatid.GotFocus
        Try
            ...
            Else
                If Cmbxspcatid.Items.Count > 0 Then
                    Cmbxspcatid.SelectedValue = DefltVatCst
                    If Me.Cmbxspcatid.SelectedIndex = -1 Then
    ...
    Private Sub DgSaleDirect_CellEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DgSaleDirect.CellEnter
        Try
            ...
            If Me.CmbxPlist.SelectedIndex = 0 Then
                Me.DgSaleDirect.CurrentRow.Cells(3).ReadOnly = False
            Else
                Me.DgSaleDirect.CurrentRow.Cells(3).ReadOnly = True
            End If
        Catch ex As Exception
    ...
    Private Sub RbTrtPart_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RbTrtPart.CheckedChanged, RbTrt2nd.CheckedChanged, RbTrt1.CheckedChanged
        Try
            If Me.RbTrtPart.Checked = True Then
                Me.BtnItmDetails.Visible = True
                Me.BtnItmDetails.Enabled = True
                'Me.GroupBox1.Top = 140
                'Me.GroupBox1.Height = 101
                Me.DgSaleDirect.MultiSelect = False
                Me.BtnSe_Save.Enabled = False
            Else
                Me.BtnItmDetails.Visible = False
                Me.BtnItmDetails.Enabled = False
                'Me.GroupBox1.Top = 153
                'Me.GroupBox1.Height = 73
                Me.DgSaleDirect.MultiSelect = True
                Me.BtnSe_Save.Enabled = True
            End If
    ...
    Private Sub BtnItmDetails_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnItmDetails.GotFocus
        Try
            If FrmShow_flag(23) = True Then
                Me.DgSaleDirect.Rows.RemoveAt(xxSelRowIndx)
                If MaxSplid > 0 Then
                    Me.TxtSaleBillNo.Text = ((MaxSplid + 1) + 5000)
                End If
                If Not (Me.DgSaleDirect.Rows.Count) > 0 Then
                    Me.Close()
                End If
                FrmShow_flag(23) = False
            End If
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Critical, "File Name :-" & Me.Name & " Private Sub BtnItmDetails_GotFocus, Line No. 2067")
        End Try
    End Sub

    Private Sub BtnItmDetails_MouseHover(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnItmDetails.MouseHover
        Try
            Me.BtnItmDetails.Font = New System.Drawing.Font("Microsoft Sans Serif", 9.0!, System.Drawing.FontStyle.Underline, System.Drawing.GraphicsUnit.Point, CType(0, Byte))
    ...
    Private Sub BtnItmDetails_MouseLeave(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnItmDetails.MouseLeave
        Try
            Me.BtnItmDetails.Font = New System.Drawing.Font("Microsoft Sans Serif", 7.0!, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, CType(0, Byte))
            Me.BtnItmDetails.ForeColor = System.Drawing.Color.DimGray
        Catch ex As Exception

        End Try
    End Sub

    Private Sub BtnItmDetails_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnItmDetails.Click
        Try
            SaveFlag1 = True
            FrmShow_flag(23) = False
            If Me.DgSaleDirect.SelectedRows.Count > 0 Then
                SBRItmId = Me.DgSaleDirect.SelectedRows(0).Cells(5).Value
                SBRqnty = Me.DgSaleDirect.SelectedRows(0).Cells(0).Value
                xxSelRowIndx = Me.DgSaleDirect.SelectedRows(0).Index
                If Not (Me.DgSaleDirect.SelectedRows(0).Cells(0).Value) > 0 Or Trim(Me.DgSaleDirect.SelectedRows(0).Cells(2).Value) = "" Then
                    MsgBox("Invalid Input!, Quantity and item name can't be blank", MsgBoxStyle.Critical, "Check Qnty. and Item Name")
                    Exit Sub
                End If
                If ChkExistingInBom(SBRItmId) = False Then
                    MsgBox("Current Item has no attachment." & Chr(13) & "It is suggested to choose an option from first Quality or second/scrap Qauality", MsgBoxStyle.Information, "No Attachment Found!!!")
                    Me.RbTrt2nd.Checked = True
                    Exit Sub
                End If

                Save_If_bomItemshandles()
                If SaveFlag1 = False Then

                    Exit Sub
                End If
                Me.RbTrt1.Enabled = False
                Me.RbTrt2nd.Enabled = False
                Dim frmSBR As New FrmSaleItmRecdBackDetails
                frmSBR.ShowInTaskbar = False
                frmSBR.ShowDialog()
            Else
                MsgBox("Invalid input!, Select an item first.", MsgBoxStyle.Critical, "File :- " & Me.Name)
            End If
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Critical, "File :- " & Me.Name & " Sub/Function :- BtnItmDetails_Click, Line No. 2083 ")
        End Try
    End Sub
    ...
    Private Function ChkExistingInBom(ByVal xxId As Integer) As Boolean
        ...
        End Try
    End Function
    Private Sub Save_If_bomItemshandles()
        Try

            Dim Qntyforbom As Double = Me.DgSaleDirect.SelectedRows(0).Cells(0).Value
            Dim Rateforbom As Double = Me.DgSaleDirect.SelectedRows(0).Cells(3).Value
            Dim ItmIdforbom As Integer = Me.DgSaleDirect.SelectedRows(0).Cells(5).Value
            ...
            TpoCmd.Parameters.AddWithValue("@SBR_itmid", ItmIdforbom)
            TpoCmd.Parameters.AddWithValue("@SBR_itmqnty", Qntyforbom)
            TpoCmd.Parameters.AddWithValue("@SBR_itmrate", Rateforbom)
            TpoCmd.ExecuteNonQuery()
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Critical, "File Name :- " & Me.Name & " Private Sub Save_If_bomItemshandles, Line No. 2177 ")
            SaveFlag1 = False
            Exit Sub
        Finally
            TpoCmd.Dispose()
        ...
        Try
            ...
            SBRCurrId = CurrPbillId
        Catch ex1 As Exception
            MsgBox(ex1.Message, MsgBoxStyle.Critical, "File Name :- " & Me.Name & " Private Sub Save_If_bomItemshandles, Line No. 2186 ")
            SaveFlag1 = False
        End Try
    ...
    Private Sub txtlbltot_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtlbltot.KeyDown
        Try
            If e.KeyCode = Keys.Enter Then
                Me.txtlbltot_Leave(sender, e)
            End If
        Catch ex As Exception
    ...
    Private Sub txtlbltot_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtlbltot.Leave
        Try
            If Len(Me.txtlbltot.Text.Trim) <> 0 Then
    ...
    Private Sub Tplitem_VisibleChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Tplitem.VisibleChanged
        Try
            If Tplitem.Visible = True Then
    ...
    Private Sub tplcustlist_VisibleChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tplcustlist.VisibleChanged
        Try
            If tplcustlist.Visible = True Then
                GrpBx1.Visible = False
            Else
                GrpBx1.Visible = True
            End If
