...
Public Class FrmCformRecveived
    ...
    Private Sub SelRecd_Fromtable_SALEENTRY()
        Try
            ...
            FinActConn.Open()
            CFcmd = New SqlCommand("Finact_SALEENTRY_CFORM_Received_Select", FinActConn)
            ...
            While CFrdr.Read
                ...
                Me.DgCFORMRECD.Rows(xx).Cells(0).Value = False
                Me.DgCFORMRECD.Rows(xx).Cells(1).Value = Format(CFrdr("Saleentdt"), "dd/MM/yyyy")
                Me.DgCFORMRECD.Rows(xx).Cells(2).Value = Trim(CFrdr("Saleentvno"))
                Me.DgCFORMRECD.Rows(xx).Cells(3).Value = FormatNumber(CFrdr("SALEENTTOTLAMT"), 2)
                Me.DgCFORMRECD.Rows(xx).Cells(4).Value = ""
                Me.DgCFORMRECD.Rows(xx).Cells(5).Value = ""
                Me.DgCFORMRECD.Rows(xx).Cells(6).Value = CInt(CFrdr("Saleentid"))
                xx += 1
            End While
        Catch ex As Exception
            MsgBox(ex.ToString)
    ...
    Private Sub BtnExit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnExit.Click
        ...
        End Try
    End Sub
    Private Sub xCFORMrECD_SAVE()
        Try
            If FinActConn.State Then FinActConn.Close()
            FinActConn.Open()
            For Each xdgr As DataGridViewRow In Me.DgCFORMRECD.Rows
                If xdgr.Cells(0).Value = True Then
                    CFcmd = New SqlCommand("Finact_SALEENTRY_CFORM_INSERT_UPDATE", FinActConn)
                    CFcmd.CommandType = CommandType.StoredProcedure
                    CFcmd.Parameters.AddWithValue("@xROWID", CInt(xdgr.Cells(6).Value))
                    CFcmd.Parameters.AddWithValue("@xCFNO", Trim(xdgr.Cells(4).Value))
                    CFcmd.Parameters.AddWithValue("@xCFREMARKS", Trim(xdgr.Cells(5).Value))
                    CFcmd.Parameters.AddWithValue("@xCFfLAG", 1)
                    CFcmd.ExecuteNonQuery()
                    CFcmd.Dispose()
                End If
            Next
            MsgBox("Current Record has been saved", MsgBoxStyle.Information, Me.Text)
            Me.DgCFORMRECD.Rows.Clear()
            Me.BtnOk.Text = "&OK"
        ...
        Try
            If Me.BtnSave.Text.Trim = "&Save" Then
                ...
                For Each xRR As DataGridViewRow In Me.DgCFORMRECD.Rows
                    If xRR.Cells(0).Value = True Then
                        xIn += 1
                    End If
                Next
                If xIn > 0 Then
                    If MessageBox.Show("Are you sure to save current record(s)?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                        xCFORMrECD_SAVE()
                    Else
                        Return
                    End If
