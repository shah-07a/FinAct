...
Public Class FrmGradMstr
    ...
    Dim Intype, FlagCelVal, FlagCelVal1 As String
    ...
    Private Sub Chk_Numericval()
        ...
        If Trim(Txthtin.Text) = "" Then
            Txthtin.BackColor = Color.PapayaWhip
            IndxConfig += 1
            Label9.Visible = True
            Label2.Visible = True
            Label9.Text = "Invalid Input!"
        Else
            Txthtin.BackColor = Color.White
    ...
    Private Sub Save_Recd()
        Try
            If Trim(BtnSave.Text) = "&Save" Then
                Itmconfig_cmd = New SqlCommand("finact_itmgradmstr_insert", FinActConn)
                Itmconfig_cmd.CommandType = CommandType.StoredProcedure
            End If
            Itmconfig_cmd.Parameters.AddWithValue("@gname", Txthtin.Text)
            ...
            Try
                If dg.SelectedRows.Count > 0 Then
                    ...
                    For i = 0 To dg.SelectedRows.Count - 1
                        Itmconfig_cmd = New SqlCommand("Delete from finact_itmgradmstr where itmgradid=@itmid", FinActConn)
    ...
    Private Sub Create_fill_datagrid()
        Try
            ...
            dg.Columns.Add("in", "Name Of Grade")
            ...
            For mk = 0 To sql.ds.Tables("size_m").Rows.Count - 1
                ...
                ' for fetching status
                cel = New DataGridViewTextBoxCell
                cel.Value = sql.ds.Tables("size_m").Rows(mk).ItemArray(2).ToString()
                ' cel.ReadOnly = True
                dgr.Cells.Add(cel)
    ...
    Private Function validate_input() As Boolean
        ...
        For i = 0 To Me.dg.Rows.Count - 1
            For j = i + 1 To Me.dg.Rows.Count - 1
                If Me.dg.Rows(i).Cells(1).Value = Me.dg.Rows(j).Cells(1).ToString Then ' And Me.dg.Rows(i).Cells(2).Value = Me.dg.Rows(j).Cells(2).Value Then
                    Return True
                End If
            Next
        Next
                    ...
                    If FlagEditAllow = True Then
                        If validate_input() Then
                            ...
                            If Trim(FlagCelVal1) <> "" Then
                                Me.dg.SelectedRows(i).Cells(1).Value = Trim(FlagCelVal1).ToString
                            End If
                            FlagEditAllow = False
                            b = 0
                            FlagCelVal1 = ""
                        ...
                        End If
                    End If
                    Itmconfig_cmd = New SqlCommand("Finact_itmgradmstr_update", FinActConn)
                    Itmconfig_cmd.CommandType = CommandType.StoredProcedure
                    Itmconfig_cmd.Parameters.AddWithValue("@itmId", Me.dg.SelectedRows(i).Cells(0).Value)
                    Itmconfig_cmd.Parameters.AddWithValue("@gname", Me.dg.SelectedRows(i).Cells(1).Value)
                    Itmconfig_cmd.Parameters.AddWithValue("@status", Me.dg.SelectedRows(i).Cells(2).Value)
                    Itmconfig_cmd.Parameters.AddWithValue("@edtusr", Me.dg.SelectedRows(i).Cells(4).Value)
    ...
    Private Function ChkExisiting_Rec() As Boolean
        Try
            ...
            Itmconfig_cmd = New SqlCommand("select itmgradname from finact_itmgradmstr where itmgradname=@in", FinActConn)
    ...
    Private Sub BtnSave_MouseHover(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.MouseHover, Btnclse.MouseHover, btndele.MouseHover, BtnFind.MouseHover
        'Dim btn As Button = CType(sender, Button)
        'btn.BackgroundImage = Global.FinAcct.My.Resources.Resources.BUTTONTOP
    End Sub

    Private Sub Btnclse_MouseLeave(ByVal sender As Object, ByVal e As System.EventArgs) Handles Btnclse.MouseLeave, btndele.MouseLeave, BtnFind.MouseLeave, BtnSave.MouseLeave

        'Dim btn As Button = CType(sender, Button)
        'btn.BackgroundImage = Global.FinAcct.My.Resources.Resources.btnimages
    End Sub
    ...
    Private Sub Txthtin_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Txthtin.KeyDown
        Try
