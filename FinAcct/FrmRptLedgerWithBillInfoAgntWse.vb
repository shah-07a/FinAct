...
Public Class FrmRptLedgerWithBillInfoAgntWse
    ...
    Dim CrRptLdgr As New CrRptLedgerWithBillInfoAgntWse
    ...
    Private Sub FrmRptGenLdgr_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        cmd = New SqlCommand("Delete from FINACT_TEMP_gl_agntwise", FinActConn)
        Try
            cmd.ExecuteNonQuery()
    ...
    Private Sub FillTempTable_Ldgr()
        Try
            ...
            FinActConn.Open()
            cmd = New SqlCommand("Finact_Rpt_LedgerWithBillInfo_AgentWise_FillTalbe", FinActConn)
    ...
    Private Sub SelRecd_Fromtable_Ldgr()
        Try
            ...
            If Not Me.CmbxLdgr.SelectedValue = -1 Then
                Selact = (Me.CmbxLdgr.SelectedValue)
            End If

            Fetch_Stock_Record_SelectedDateRange_withSelITem_and_BalType("Finact_Rpt_Ledger_Agntwise_Select", Me.CrRptLdgr, Me.CrRptVewLdgr, Me.DtpLdgr1.Value.Date, Me.DtpLdgr2.Value.Date, Selact, Stype, Sbtype)
