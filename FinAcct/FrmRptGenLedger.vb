...
Public Class FrmRptGenLedger
    ...
    Dim CrRptLdgr As New CrRptGenLedger
    ...
    Private Sub FrmRptGenLdgr_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        cmd = New SqlCommand("Delete from FINACT_TEMP_genledger", FinActConn)
        Try
            cmd.ExecuteNonQuery()
    ...
    Private Sub FrmRptGenLdgr_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Fill_Combobox_where_Not_In_cond4("splrid", "Splrname", "Splrtype", "Finactsplrmstr", "Customer", "Vendor", "Bank", "Cash Book", Me.CmbxLdgr, "SplrDelStatus", CInt(1))
            FillTempTable_Ldgr()
            Me.DtpLdgr1.Focus()
        Catch ex As Exception
    ...
    Private Sub FillTempTable_Ldgr()
        Try
            ...
            FinActConn.Open()
            cmd = New SqlCommand("Finact_Rpt_GenLedger_FillTalbe", FinActConn)
    ...
    Private Sub SelRecd_Fromtable_Ldgr()
        Try
            ...
            Fetch_Stock_Record_SelectedDateRange_withSelITem_and_BalType("Finact_Rpt_GenLedger_Select", Me.CrRptLdgr, Me.CrRptVewLdgr, Me.DtpLdgr1.Value.Date, Me.DtpLdgr2.Value.Date, Selact, Stype, Sbtype)
