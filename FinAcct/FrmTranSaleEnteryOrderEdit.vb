...
Public Class FrmTranSaleEnteryOrderEdit
    ...
    Private Sub FrmTranSaleOrder_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            End If
            xFlag = True
            Me.Text = "Sale Entry (Through  Sale Order) Edit-Mode  " & "  (CURRENT USER NAME:- " & Trim(Current_UserName) & ",DESIGNATION:- " & Trim(Current_UserDesi)
    ...
    Private Sub LstVewItem_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles LstVewItem.DoubleClick
        Try
            If Me.LstVewItem.SelectedItems.Count > 0 Then
                ...
                Me.DgSaleDirect.CurrentRow.Cells(4).Value = Trim(LstVewItem.SelectedItems.Item(0).SubItems(7).Text)
                Me.Tplitem.Location = New System.Drawing.Point(837, 243)
                Me.Tplitem.Visible = False
                Me.Tplitem.Enabled = False
                Me.DgSaleDirect.Focus()
    ...
    Private Sub BtnPe_Save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSe_Save.Click
        Try
            ...
            For Xx = 0 To Me.DgSaleDirect.Rows.Count - 1
                If Trim(Me.DgSaleDirect.Rows(Xx).Cells(0).ErrorText) <> "" Then
    ...
    Private Sub SaleEntryupdate()
        If Me.DgSaleDirect.Rows.Count >= 1 Then
            Try
                TpoCmd = New SqlCommand("Finact_Saleentry_update", FinActConn)
                TpoCmd.CommandType = CommandType.StoredProcedure
            ...
            Try
                Dim DgCntr As Integer
                For DgCntr = 0 To Me.DgSaleDirect.Rows.Count - 1
                    If Me.DgSaleDirect.Rows(DgCntr).Cells(0).Value > 0 Then
                        TpoCmd = New SqlCommand("Finact_SaleEntryRecd_Edit_Details_Update", FinActConn)
                        TpoCmd.CommandType = CommandType.StoredProcedure
                        TpoCmd.Parameters.AddWithValue("@dSaleid", Trim(Me.DgSaleDirect.Rows(DgCntr).Cells(8).Value))
                        TpoCmd.Parameters.AddWithValue("@dSalerecdqnty", CDbl(Me.DgSaleDirect.Rows(DgCntr).Cells(0).Value))
                        TpoCmd.Parameters.AddWithValue("@dSaleqntyrecddt", Now)
                        TpoCmd.Parameters.AddWithValue("@dSaleitmdiscr", Trim(Me.DgSaleDirect.Rows(DgCntr).Cells(2).Value))
                        TpoCmd.Parameters.AddWithValue("@dSaleitmrate", CDbl(Me.DgSaleDirect.Rows(DgCntr).Cells(3).Value))
                        TpoCmd.Parameters.AddWithValue("@dSaleOldrecdqnty", CelEntVal(DgCntr))
                        TpoCmd.ExecuteNonQuery()
                    End If
                Next
                xUpdateFlag = False
        ...
        Try
            If e.ColumnIndex = 0 Then
                If Cl_Kbl = True Then
                    ...
                    Dim xStk As Double = SumOf_In_and_Outward_Items(xCurItemId, xCurItemId)
                    If Me.DgSaleDirect.CurrentRow.Cells(0).Value > SumOf_In_and_Outward_Items(xCurItemId, xCurItemId) Then
                    ...
                    Else
                        ...
                        Me.DgSaleDirect.CurrentRow.DefaultCellStyle.BackColor = Color.GhostWhite
                    End If
                Else
                    CelLevVal = Me.DgSaleDirect.CurrentRow.Cells(0).Value
                    If (CelEntVal(Me.DgSaleDirect.CurrentRow.Index) < CelLevVal) Then
                        Me.DgSaleDirect.CurrentRow.Cells(0).ErrorText = "Quantity should not be greater than" & " " & CelEntVal(Me.DgSaleDirect.CurrentRow.Index)
                    ElseIf CelLevVal = 0 Then
                        Me.DgSaleDirect.CurrentRow.Cells(0).ErrorText = "Quantity should be greater than ZERO"
                    Else
                        Me.DgSaleDirect.CurrentRow.Cells(0).ErrorText = ""
                    End If
                End If
                ...
                If e.ColumnIndex = 0 Then
                    If (Me.DgSaleDirect.CurrentRow.Cells(0).Value) > 0 Then
                        ...
                        For Rc = 0 To Me.DgSaleDirect.Rows.Count - 1
                            ...
                            If Trim(Me.Cmbxdistype.Text) = "Discount Percentage" Then
    ...
    Private Sub CreateGridColumns_Saleentr_Edit()
        Try
            ...
            If TpoRdr1.IsDBNull(0) = False Then
                ...
                Me.DtpSaleDue.Value = CurrDate.Date
                ...
                Prordid = Trim(TpoRdr1("Saleentorderid"))
                SpCid = TpoRdr1("Saleentvatrate")
                ...
                Dim DisvL As Double = CDbl(Me.Mtxtdisvalue.Text)
        ...
        Try
            ...
            If Cl_Kbl = True Then
                DgItms.Columns(3).ReadOnly = False
            Else
            ...
            'DgItms.Columns.Add("ColStatus", "itemcode")
            DgItms.Columns(9).HeaderText = "Carton No."
            DgItms.Columns(9).Name = "xCartNo"
            DgItms.Columns(9).Width = 90
            DgItms.Columns(9).Visible = True
            DgItms.Columns(9).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft

            DgItms.Columns.Add("ColInStok", "Sotck In Hand")
            ''DgItms.Columns(10).HeaderText = "Sotck In Hand"
            ''DgItms.Columns(10).Name = "ColInStok"
            ...
            'DgItms.Columns(11).HeaderText = "Edit Flag"
            'DgItms.Columns(11).Name = "ColresStok"
            DgItms.Columns(11).Width = 10
            DgItms.Columns(11).Visible = False
            'DgItms.Columns(11).DefaultCellStyle.Format = "No"
            DgItms.Columns(11).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft
            DgItms.Columns(11).DefaultCellStyle.NullValue = Nothing
            Dim rc As Integer = 0
            ReDim CelEntVal(Me.DgSaleDirect.Rows.Count - 1)

            For rc = 0 To Me.DgSaleDirect.Rows.Count - 1
                Me.DgSaleDirect.Rows(rc).Cells(11).Value = 1 '1 is  satand for already existed record.
                CelEntVal(rc) = Me.DgSaleDirect.Rows(rc).Cells(0).Value
            Next
        ...
        Finally
            ...
            TpoAdptr.Dispose()
            lbltoc.Text = FormatNumber(SumOf_Txtvalues(), 2)
            Me.DgSaleDirect.Columns(Me.DgSaleDirect.ColumnCount - 1).Visible = False
            Me.DgSaleDirect.Focus()
    ...
    Private Function XSelect_from_Xsplrmstr_Xinfo(ByVal SpType As String, ByVal Spid As Integer) As Boolean
        Try
            ...
            If TpoRdr.IsDBNull(0) = False Then
                dy1 = TpoRdr(6)
                If dy1 > 0 Then
                    Me.DtpSaleDue.Value = Me.dtpordrdt.Value.AddDays(dy1)
                Else
  ...
  , ChkbOthrCharg.KeyDown, ChkVatInfo.KeyDown, CmbxCarri.KeyDown, Cmbxdistype.KeyDown, Cmbxitem.KeyDown, Cmbxspcatid.KeyDown _
  ...
  , TxtVatinvno.KeyDown, txtvCode.KeyDown, TxtVname.KeyDown, MTxtTotlamt.KeyDown, MtxtfrtChargs.KeyDown, mtxtulcharg.KeyDown, MtxtPkgcharg.KeyDown, Chkprnt.KeyDown
