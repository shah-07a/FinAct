...
Imports System.IO
Public Class FrmDlyProdMatrlChck
    ...
    Private Sub FrmDlyProdMatrlChck_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
            xDel_xTables()
    ...
    Private Sub AllControls_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles DtpMovedt.KeyDown, Rbxicode.KeyDown, RbxiName.KeyDown, Txtitmcode.KeyDown, txtmovqnty.KeyDown, txtremark.KeyDown
        Try
    ...
    Private Sub BtnMMSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnMMSave.Click
        Try
            ...
            If MM_Indx <> 0 Then
                ...
                Exit Sub
            Else
                If MessageBox.Show("Are you sure to print current MRP", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.No Then
                    Return
                Else
                    xDel_xTables()
                    xCurnt_xRec_xSav_xMmove()
                    Dim FrmMrR As New FrmCrRptMRP
                    FrmMrR.ShowInTaskbar = False
                    FrmMrR.ShowDialog()
                    FrmMrR.StartPosition = FormStartPosition.CenterParent
                End If
    ...
    Private Sub xCurnt_xRec_xSav_xMmove()
        ...
        Try
            For SetSave = 0 To Me.Lstvewitmall.Items.Count - 1
                ...
                MM_cmd = New SqlCommand("FinAct_Rpt_Temp_MRP_FillTable", FinActConn)
                MM_cmd.CommandType = CommandType.StoredProcedure
                MM_cmd.Parameters.AddWithValue("@MrpIndx", SetSave)

                MM_cmd.Parameters.AddWithValue("@mrpdt", Me.DtpMovedt.Value.Date)
                ''MM_cmd.Parameters.AddWithValue("@mmbomid", MM_ItemId)
                MM_cmd.Parameters.AddWithValue("@Mrpqnty", CDbl(Me.txtmovqnty.Text))
                MM_cmd.Parameters.AddWithValue("@mrpicode", Trim(Me.Txticode.Text))
                MM_cmd.Parameters.AddWithValue("@mrpiname", Trim(Me.lbldesc.Text))
                MM_cmd.Parameters.AddWithValue("@mrpunt", Trim(Me.lblunit.Text))

                MM_cmd.Parameters.AddWithValue("@mrpMin", CDbl(Me.lblmin.Text))
                MM_cmd.Parameters.AddWithValue("@mrpMax", CDbl(Me.lblmax.Text))
                MM_cmd.Parameters.AddWithValue("@mrpSiH", CDbl(Me.lblsih.Text))
                MM_cmd.Parameters.AddWithValue("@mrpResrv", CDbl(Me.LblResStock.Text))
                MM_cmd.Parameters.AddWithValue("@mrpNet", CDbl(Me.LblNet.Text))
                MM_cmd.Parameters.AddWithValue("@mrpremrk", Trim(Me.txtremark.Text))

                '----
                MM_cmd.Parameters.AddWithValue("@xmrpIname", Trim(Me.Lstvewitmall.Items(SetSave).SubItems(0).Text))
                MM_cmd.Parameters.AddWithValue("@mrplocation", Trim(Me.Lstvewitmall.Items(SetSave).SubItems(1).Text))
                MM_cmd.Parameters.AddWithValue("@mrpQntyPu", CDbl(Me.Lstvewitmall.Items(SetSave).SubItems(2).Text))
                MM_cmd.Parameters.AddWithValue("@mrpQntyR", CDbl(Me.Lstvewitmall.Items(SetSave).SubItems(3).Text))
                MM_cmd.Parameters.AddWithValue("@mrpQntySiH", CDbl(Me.Lstvewitmall.Items(SetSave).SubItems(4).Text))
                MM_cmd.Parameters.AddWithValue("@mrpQntyInDemnd", CDbl(Me.Lstvewitmall.Items(SetSave).SubItems(5).Text))
                MM_cmd.Parameters.AddWithValue("@mrpQntyAvalabl", CDbl(Me.Lstvewitmall.Items(SetSave).SubItems(6).Text))
                MM_cmd.Parameters.AddWithValue("@mrpNetStatus", CDbl(Me.Lstvewitmall.Items(SetSave).SubItems(7).Text))
    ...
    Public Function FindMax_MovId() As Integer
        ...
        End Try
    End Function
    Private Sub xDel_xTables()
        Try
            MM_cmd = New SqlCommand("FinAct_Rpt_Temp_MRP_DeleteTable", FinActConn1)
            MM_cmd.CommandType = CommandType.StoredProcedure
            MM_cmd.ExecuteNonQuery()
        Catch ex As Exception
            MsgBox(ex.Message)
                        ...
                        While MM_rdr.Read = True
                            If MM_rdr.IsDBNull(0) = False Then
                                Try
                                    ...
                                    If xItype = "Trading" Or xItype = "Purchase" Then
