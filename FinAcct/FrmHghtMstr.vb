...
Public Class FrmHghtMstr
    ...
    Private Sub Save_Recd()
        Try
            ...
            If Trim(BtnSave.Text) = "&Save" Then
                Itmconfig_cmd = New SqlCommand("finact_itmhgtmstr_insert", FinActConn)
            ...
            IntHtCmMm(1) = Trim(Txthtin.Text) ' for  Stock item master
            IntHtCmMm(2) = Trim(Txthtin.Text) ' for stock bom
            ClearValue()
            ...
            Try
                If dg.SelectedRows.Count > 0 Then
                    ...
                    For i = 0 To dg.SelectedRows.Count - 1
                        Itmconfig_cmd = New SqlCommand("Delete from finact_itmhgtmstr where itmhtid=@itmid", FinActConn)
            ...
            If b > 0 Then
                For i = 0 To dg.SelectedRows.Count - 1
                    If FlagEditAllow = True Then
                        ...
                        End If
                    End If
                    Itmconfig_cmd = New SqlCommand("Finact_itmhgtmstr_update", FinActConn)
                    Itmconfig_cmd.CommandType = CommandType.StoredProcedure
                    Itmconfig_cmd.Parameters.AddWithValue("@itmId", Me.dg.SelectedRows(i).Cells(0).Value)
                    ...
                    Itmconfig_cmd.Parameters.AddWithValue("@edtusr", Current_UsrId)
                    Itmconfig_cmd.Parameters.AddWithValue("@edtdt", Now)
                    IntHtCmMm(1) = Trim(Me.dg.SelectedRows(i).Cells(1).Value) 'for Stock item master
                    IntHtCmMm(2) = Trim(Me.dg.SelectedRows(i).Cells(1).Value) ' for stock bom
                    Itmconfig_cmd.ExecuteNonQuery()
                Next
    ...
    Private Function ChkExisiting_Rec() As Boolean
        Try
            ...
            Else
                Intype = "Mm"
            End If
            Itmconfig_cmd = New SqlCommand("select itmhtin,itmhttype from finact_itmhgtmstr where itmhtin=@in and itmhttype=@type", FinActConn)
    ...
    Private Sub Allcontrols_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Txthtin.KeyDown, RBCm.KeyDown, _
    RBin.KeyDown, Rbmm.KeyDown
        Try
