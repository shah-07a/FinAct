...
Public Class FrmCrRptPurRegstr
    ...
    Dim CrRptPurReg As Object
    Dim xVATRate As Double = 0

    Private Sub FrmCrRptPurRegstr_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        ...
        End Try
    End Sub
    Private Sub FrmCrRptPurRegstr_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Me.Size = New Point(Screen.PrimaryScreen.WorkingArea.Width - 30, 124)
            Me.Top = 0
            Me.Left = 0
    ...
    Private Sub SelRecd_Fromtable_StokIssue()
        Try
            ...
            Fetch_SalePur_Record_SelectedDateRange_withSelITems("Finact_Rpt_TEMP_PurEntry_AsperSelectRange_Select", Me.CrRptPurReg, Me.CrRptVewPurRegstr, Me.DtpLdgr1.Value.Date, Me.DtpLdgr2.Value.Date, xCustmrId, xTaxVal, xxSelTaxCat, xxSelCustRange)
            SetExtraPramtoCrRpt()
            If Me.ChkRptLdgr.Checked = True Then
                CoprofileParamsRest_Adrs(Me.CrRptPurReg, Me.CrRptVewPurRegstr, True, True, Me.DtpLdgr1, Me.DtpLdgr2)
            Else
                CoprofileParamsRest_Adrs(Me.CrRptPurReg, Me.CrRptVewPurRegstr, True, False, Me.DtpLdgr1, Me.DtpLdgr2)
            End If
    ...
    Private Sub ChkRptLdgr_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkRptLdgr.CheckedChanged
        Try
            If xCrRptPurchase = True Then
                If Cl_TimeIndia = True Or Cl_TimeRubber = True Then
                    CrRptPurReg = New CrRptPurRegstrSumry
                Else
                    CrRptPurReg = New CrRptPurSummryRegstrKblVATType
                End If

            Else
                If Cl_TimeIndia = True Or Cl_TimeRubber = True Then
                    CrRptPurReg = New CrRptPurRegstr
                Else
                    CrRptPurReg = New CrRptPurRegstrKblType
                End If
   ...
   , ChkRptLdgr.KeyDown, DtpLdgr2.KeyDown, ChkBxTaxCat.KeyDown
    ...
    Private Sub SetExtraPramtoCrRpt()
        Try
            ...
            Me.CrRptPurReg.DataDefinition.ParameterFields("vattin").ApplyCurrentValues(ParmVal1)
            CrRptVewPurRegstr.ReportSource = CrRptPurReg
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
            cmd.Dispose()
