...
Public Class FrmSaleItmRecdBackDetailsEdit
            ...
            If SaveFlag = False Then
                If MessageBox.Show("You are going to close current page without saving any record. Are you sure to close?", "Page closing.....!", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                    ...
                    Try
                        ...
                        FinActConn1.Open()
                        SBRcmd1 = New SqlCommand("Finact_SBR_Ifcommandisunsuccess_3_Delete", FinActConn1)
                        SBRcmd1.CommandType = CommandType.StoredProcedure
                        SBRcmd1.Parameters.AddWithValue("@ConIdxx", SBRBillNo)
    ...
    Private Sub fill_DgView1(ByVal curitmid As Integer)
       
        Try
            If FinActConn.State Then FinActConn.Close()
            FinActConn.Open()
            SBRcmd = New SqlCommand("Finact_FinactSaleItemBackRecdEntry_Details__Where_Concrnid_Edit", FinActConn)
            SBRcmd.CommandType = CommandType.StoredProcedure
            SBRcmd.Parameters.AddWithValue("@SaleentConcrnid", curitmid)
            SBRrdr = SBRcmd.ExecuteReader
            Dim x As Integer
            x = 0
            While SBRrdr.Read()
                If SBRrdr.HasRows = True Then
                    Me.DgSBRItems1.Rows.Add()
                    Me.DgSBRItems1.Rows(x).Cells(0).Value = SBRrdr(1)
                    Me.DgSBRItems1.Rows(x).Cells(1).Value = SBRrdr(2)
                    Me.DgSBRItems1.Rows(x).Cells(2).Value = SBRrdr(4)
                    Me.DgSBRItems1.Rows(x).Cells(3).Value = FormatNumber(SBRrdr(3), 2)
                    Me.DgSBRItems1.Rows(x).Cells(4).Value = FormatNumber(SBRrdr(5), 2)
                    Me.DgSBRItems1.Rows(x).Cells(5).Value = FormatNumber(SBRrdr(0), 4)
                    Me.DgSBRItems1.Rows(x).Cells(6).Value = SBRrdr(9)
                    Me.DgSBRItems1.Rows(x).Cells(7).Value = SBRrdr(6) 'Rec id
                    x += 1
                End If
            End While
        Catch ex As SqlException
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        Try
            ...
            If Trim(Me.BtnSave.Text) = "&Update" Then
    ...
    Private Sub SBRItemsSave()
        Try
            ...
            For DgCntr = 0 To Me.DgSBRItems.Rows.Count - 1
                ...
                If Qnty6 > 0 Then
                    ...
                    SBRcmd.CommandType = CommandType.StoredProcedure
                    SBRcmd.Parameters.AddWithValue("@dPurentconcrnid", (SBRid))
                    ...
                    SBRcmd.Parameters.AddWithValue("@dPurent_Isprod", 3)
                ...
                If Qnty7 > 0 Then
                    ...
                    SBRcmd.CommandType = CommandType.StoredProcedure
                    SBRcmd.Parameters.AddWithValue("@d_SBR_entconcrnid", (SBRid))
                    ...
                    SBRcmd.Parameters.AddWithValue("@d_SBR_ent_isprod", 3)
                    SBRcmd.ExecuteNonQuery()
                    SBRcmd.Dispose()
                End If
        ...
        Catch ex As Exception
            ...
            Try
                SBRcmd1 = New SqlCommand("Finact_SBR_Ifcommandisunsuccess_1a_Delete", FinActConn1)
                SBRcmd1.CommandType = CommandType.StoredProcedure
                SBRcmd1.Parameters.AddWithValue("@ConIdxx", SBRid)
                SBRcmd1.ExecuteNonQuery()
            Catch ex1 As Exception
    ...
    End Sub
    Private Sub updatePrvsRecord(ByVal Recid As Integer)
        Try
            SBRcmd1 = New SqlCommand("Finact_SBR_DeleteandUpdate", FinActConn1)
            SBRcmd1.CommandType = CommandType.StoredProcedure
            SBRcmd1.Parameters.AddWithValue("@ConIdxx", Recid)
            SBRcmd1.Parameters.AddWithValue("@SBRBill", SBRBillNo)
            SBRcmd1.Parameters.AddWithValue("@edtusrid", Current_UsrId)
            SBRcmd1.Parameters.AddWithValue("@edtdt", Now)
            SBRcmd1.ExecuteNonQuery()
        Catch ex1 As Exception
            MsgBox(ex1.Message, MsgBoxStyle.Critical, "File Name:- " & Me.Text & "  Private Sub SBRItemsSave, Line No. 255")
        Finally
